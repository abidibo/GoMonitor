...
func InsertLog(user string, totalMinutes int) (int64, error) {
	result, err := db.DB().C.Exec("INSERT INTO log (user, partial_time_minutes) VALUES (?, ?)", user, totalMinutes)
...
func GetTotalTodayTimeMinutes(user string) (int, error) {
	stm, err := db.DB().C.Prepare("SELECT SUM(partial_time_minutes) FROM log WHERE user = ? AND date(timestamp) = date('now')")
	if err != nil {
...
func GetTotalDateTimeMinutes(user string, date string) (int, error) {
	stm, err := db.DB().C.Prepare("SELECT SUM(partial_time_minutes) FROM log WHERE user = ? AND date(timestamp) = ?")
	if err != nil {
		logger.ZapLog.Error("Cannot get total day time", err)
		return 0, err
	} else {
...
func GetTotalProcessTimeMinutes(user string, processName string, date string) (int, error) {
	// all processes
	// SELECT DISTINCT name, SUM(partial_time_minutes) as tot FROM (SELECT p.id, p.log_id, p.name, l.partial_time_minutes from log_process as p, log as l WHERE l.user = 'abidibo' AND date(l.timestamp) = '2023-09-25' AND p.log_id = l.id GROUP BY p.log_id, p.name) GROUP BY name ORDER BY tot DESC;
	stm, err := db.DB().C.Prepare("SELECT SUM(partial_time_minutes) FROM (SELECT p.log_id, p.name, l.partial_time_minutes from log_process as p, log as l WHERE p.name = ? and l.user = ? AND date(l.timestamp) = ? AND p.log_id = l.id GROUP BY p.log_id, p.name)")
	if err != nil {
